...
export function BlogFeatured() {
  return (
    <div className="mt-12">
      <h2 className="text-3xl font-bold mb-8 text-foreground dark:text-white">Featured Articles</h2>
      <div className="grid md:grid-cols-2 gap-6">
        {featuredPosts.map((post) => (
          <Link key={post.id} href={`/blog/${post.slug}`}>
            <Card className="overflow-hidden h-full hover:shadow-lg transition-shadow border-border dark:border-gray-800">
              <motion.div whileHover={{ scale: 1.05 }} transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}>
                <div className="aspect-[16/9] relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-background/80 to-transparent dark:from-[#0f172a]/80" />
                  <Badge className={cn("absolute top-4 left-4 border-none text-white", post.color)}>
                    {post.category}
                  </Badge>
                </div>
                <CardContent className="p-6">
                  <h3 className="text-2xl font-bold mb-2 line-clamp-2 text-foreground dark:text-white">{post.title}</h3>
                  <p className="text-muted-foreground dark:text-gray-300 line-clamp-2">{post.excerpt}</p>
                </CardContent>
              </motion.div>
            </Card>
          </Link>
