...
export function BookmarkButton({ postId }: BookmarkButtonProps) {
  ...
  return (
    <Button
      variant="ghost"
      size="icon"
      className="h-8 w-8 bg-background/80 dark:bg-[#0f172a]/80 backdrop-blur-sm hover:bg-background/90 dark:hover:bg-[#0f172a]/90"
      onClick={toggleBookmark}
    >
      <motion.div initial={false} animate={isBookmarked ? { scale: [1, 1.2, 1] } : { scale: 1 }}>
        <Bookmark className={cn("h-4 w-4", isBookmarked ? "fill-[#3b82f6] text-[#3b82f6]" : "fill-none")} />
      </motion.div>
